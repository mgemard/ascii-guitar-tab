 ...
 */
function parseTabLines(tabLines, metadata) {
 ...
 */
function colToBeat(col, metadata) {
    return col * 0.25; // 16th note grid
}
/**
 * Convert string + fret into frequency
 */
function getFrequency(tuningNote, fret, capo) {
 ...
 */
export async function playTab(parsed) {
    const audioContext = new AudioContext();
    const secondsPerBeat = 60 / parsed.metadata.tempo;
    const now = audioContext.currentTime + 0.1;
    let latestEndTime = now;
    for (const note of parsed.notes) {
        const tuningNote = parsed.metadata.tuning[parsed.metadata.tuning.length - 1 - note.stringIndex];
        const frequency = getFrequency(tuningNote, note.fret, parsed.metadata.capo);
